...
def forward(x):
    ...
    liner     = tf.reduce_sum(tf.gather(w, x), axis=1) + w0
    v         = get_weight([V_SIZE, FEATURE_DIM], "v")
    emb       = tf.gather(v, x)
    rows, columns = [], []
    for r, c in itertools.combinations(range(FEATURE_SIZE), 2):
        rows.append(r)
        columns.append(c)
    p         = tf.gather(emb, rows, axis=1)
    q         = tf.gather(emb, columns, axis=1)
    cross     = p * q   # batch * (m(m-1)/2) * k
    att_w1    = get_weight([FEATURE_DIM, ATTENTION_FACTOR], "att_w1")
    att_b1    = get_bias([1, ATTENTION_FACTOR], "att_b1")
    att       = tf.nn.relu(tf.add(tf.matmul(cross, att_w1), att_b1))
    att_w2    = get_weight([ATTENTION_FACTOR, 1], "att_w2")
    att_b2    = get_bias([1, ATTENTION_FACTOR], "att_b2")
    att_score = tf.nn.softmax(tf.add(tf.matmul(att, att_w2), att_b2), axis=1)
    embedding = tf.reduce_sum(cross * att_score, axis=1)
    ...
    b3        = get_bias([1, 1], "b3")
    y         = tf.add(tf.matmul(output, w3), b3) + liner
